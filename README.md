...
2. 第 `2` 遍视频，列举本次`功能`要完成的`功能点`。
3. 关掉视频，思考`功能点`，然后写出`测试用例`代码，这个过程可能会卡壳，可以回到视频指定位置学习，确保`测试用例`准确。
4. 根据`测试驱动开发`的思路，为每一个`测试用例`开发一段`功能代码`。
5. 通过 `jest` 执行每一个`测试用例`，根据结果修正`功能代码`，直到所有`测试用例`通过。
6. 沉淀所有内容，输出文章。
